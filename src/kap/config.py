...
class KapConfig(BaseModel):
    ...
    timeout_s: float = Field(default=8.0, description="Legacy total HTTP timeout in seconds")
    connect_timeout_s: float = Field(default=1.5, gt=0, description="HTTP connection timeout in seconds")
    read_timeout_s: float = Field(default=5.0, gt=0, description="HTTP response read timeout in seconds")
    ...
    request_deadline_s: float = Field(default=8.0, gt=0, description="Total deadline including retries and backoff")
    ...
    cache_expiry_default: int = Field(default=3600, description="Default cache TTL in seconds (1 hour)")
    cache_expiry_companies: int = Field(default=86400 * 3, description="Company list cache TTL (3 days)")
    cache_expiry_indices: int = Field(default=86400, description="Indices cache TTL (1 day)")
    cache_expiry_sectors: int = Field(default=86400, description="Sectors cache TTL (1 day)")
    cache_expiry_financials: int = Field(default=86400 * 7, description="Financial statements cache TTL (7 days)")
    cache_expiry_latest: int = Field(default=20, description="Latest disclosure cache TTL (seconds)")
    cache_expiry_today: int = Field(default=30, description="Today disclosure cache TTL (seconds)")
    cache_expiry_company_general: int = Field(default=86400 * 2, description="Company profile cache TTL (2 days)")
    cache_expiry_disclosure_detail: int = Field(default=86400 * 30, description="Immutable disclosure detail cache TTL (30 days)")
    ...
    registry_min_records: int = Field(default=100, ge=1, description="Minimum rows required before accepting a live company registry")
