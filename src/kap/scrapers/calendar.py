...
class CalendarScraper:
    ...
    def _parse_expected_rows(self, rows: list[dict[str, Any]]) -> list[ExpectedDisclosure]:
        ...
        for r in rows:
            ...
            title = r.get("kapMemberTitle") or r.get("companyTitle")
            ...
            if not (member or stock_code) or not (subject or period or start or end):
                continue
            ...
            company_key = str(member or stock_code).upper()
            canonical = (
                company_key,
        ...
        expected.sort(key=lambda item: (item.start_date or "9999-99-99", item.stock_code or item.company_id or ""))
        return expected
